...
public class MultiOf3_MultQ3 {

	//following segment tree algo
	public static void main(String[] args) throws Exception {
		MultiOf3_MultQ3 t = new MultiOf3_MultQ3();
	...
	public void constructTree(int[] inArr, Integer[] segArr, int start,
		...
		if(segArr[2*pos+1]!=0 || segArr[2*pos+2]!=0)
	...
	public void writeVals(Integer[] segArr, int qstart, int qend, int start,
		...
		if(start==end){
			if("total".equals(overlap)){
				...
				int parent=(pos-1)/2;
				int tmp=0;
				if(segArr[2*parent+1]%3==0 && segArr[2*parent+2]%3==0){
					tmp=segArr[parent];
					segArr[parent]=2;
					tmp=segArr[parent]-tmp;
					parent=(parent-1)/2;
					while(parent!=0){
						segArr[parent]=segArr[parent]+tmp;
						parent=(parent-1)/2;
					}
					segArr[parent]=segArr[parent]+tmp;
				}
				else if(segArr[2*parent+1]%3!=0 && segArr[2*parent+2]%3!=0){
					tmp=segArr[parent];
					segArr[parent]=0;
					tmp=tmp-segArr[parent];
					parent=(parent-1)/2;
					while(parent!=0){
						segArr[parent]=segArr[parent]-tmp;
						parent=(parent-1)/2;
	...
	public Integer readCoins(Integer[] segArr, int qstart, int qend, int start,
		...
		else if("total".equals(overlap) && start==end){
			if(segArr[pos]%3==0)
				return 1;
		}
